...
def api_docs(item, skip=[], prefix='', subclass_of=None, write=True, members=[pydoc.inspect.ismethod, pydoc.inspect.isfunction, pydoc.inspect.isdatadescriptor]):
    ...
    for fm in pydoc.inspect.getmembers(item, check_member):
        ...
        if pydoc.inspect.getdoc(fm[1]):
            output.append('\n')
            docstring = pydoc.HTMLDoc().markup(pydoc.inspect.getdoc(fm[1])).replace("<strong>", "").replace("</strong>", "")
...
if __name__ == '__main__':
    ...
    print("CREATING API DOCS FOR NPARRAY VERSION: {}".format(nparray.__version__))

    fms = api_docs(nparray, skip=[], members=[pydoc.inspect.isfunction])

    # fms = api_docs(nparray._wrappers.ArrayWrapper)
    fms = api_docs(nparray._wrappers.Array)
    fms = api_docs(nparray._wrappers.Arange)
    fms = api_docs(nparray._wrappers.Linspace)
    fms = api_docs(nparray._wrappers.Logspace)
    fms = api_docs(nparray._wrappers.Geomspace)
    fms = api_docs(nparray._wrappers.Full)
    fms = api_docs(nparray._wrappers.Zeros)
    fms = api_docs(nparray._wrappers.Ones)
    fms = api_docs(nparray._wrappers.Eye)
